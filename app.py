...
def transcribe_audio(openai_client: OpenAI, file_bytes: bytes, ext: str, language: str) -> str:
    """Transcribe audio using OpenAI Whisper. Uses ffmpeg subprocess for
    memory-efficient compression, then chunks if still > WHISPER_CHUNK_LIMIT_MB."""
    ...
    # Step 1 — write to disk then compress via ffmpeg (memory-efficient, no Python OOM)
    with tempfile.NamedTemporaryFile(delete=False, suffix=f".{ext}") as tmp_in:
        tmp_in.write(file_bytes)
        tmp_in_path = tmp_in.name
    tmp_out_path = tmp_in_path.rsplit(".", 1)[0] + "_compressed.mp3"

    subprocess.run(
        ["ffmpeg", "-y", "-i", tmp_in_path, "-ac", "1", "-ab", "64k", "-f", "mp3", tmp_out_path],
        stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True
    )
    os.unlink(tmp_in_path)
    size_mb = os.path.getsize(tmp_out_path) / (1024 * 1024)

    # Step 2 — single request if small enough
    if size_mb <= WHISPER_CHUNK_LIMIT_MB:
        srt, _ = call_whisper(tmp_out_path)
        os.unlink(tmp_out_path)
        return srt

    # Step 3 — chunk if still too large
...
def enrich_srt_in_batches(or_client: OpenAI, segments: list[dict], progress_bar, batch_size: int = 25) -> str:
    annotated_parts = []
...
with col_upload:
    uploaded_file = st.file_uploader(
        ...
        help="Up to 300 MB. Large files are automatically compressed and processed.",
    )
    if uploaded_file:
...
with col_meta:
    episode_name = st.text_input("Episode name (used in filenames)", placeholder="Episode 01 – The Beginning")
    ...
    if file_ok:
        generate_clicked = st.button("🚀 Generate Morphic Script", type="primary", use_container_width=True)
    else:
...
if file_ok and generate_clicked:
    ...
    with step1:
        try:
            ...
            st.write(f"Compressing {uploaded_file.name} ({size_mb:.1f} MB) via ffmpeg…")
    ...
    with step2:
        try:
            ...
            step2.update(label=f"✅ Step 2 — Morphic prompts added to {total} segments!", state="complete")
        except Exception as e:
    ...
    with step3:
        try:
            ...
            step3.update(label=f"✅ Step 3 — Found {n_chars} character(s) and {n_locs} location(s)!", state="complete")
    ...
    with dcol1:
        st.download_button("📄 Annotated SRT", data=annotated_srt,
            file_name=f"{safe_name}_morphic.srt", mime="text/plain", use_container_width=True)
        st.caption("Time-coded script with [MORPHIC] camera prompts per line")
    with dcol2:
        if char_sheet:
            st.download_button("👤 Character & Costume Sheet", data=char_sheet,
                file_name=f"{safe_name}_characters.txt", mime="text/plain", use_container_width=True)
    ...
    with dcol3:
        if loc_sheet:
            st.download_button("📍 Locations Sheet", data=loc_sheet,
                file_name=f"{safe_name}_locations.txt", mime="text/plain", use_container_width=True)
